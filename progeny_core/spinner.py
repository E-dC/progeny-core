...
class PortManager(object):
    ...
    def __init__(self, port_range: Union[range, Tuple[int, int], List[int]]):
    ...
    def update_available_ports(self) -> None:
        self.available_ports = set(
            [p for p in self.port_range if not self.is_port_in_use(p)]
        )
    ...
    def validate_port_range(
        cls, port_range: Union[range, Tuple[int, int], List[int]]
    ) -> range:
...
class ProdigyAdapter(object):
    ...
    def __init__(
        self, recipe_dir: Optional[str] = None, prebaked_dir: Optional[str] = None
    ):
    ...
    @classmethod
    def parse_prebaked(cls, data: Dict[Any, Any]) -> Tuple[str, Dict[str, Any], str]:
        name = data["name"]
    ...
    @classmethod
    def are_args_none(cls, data: Dict[str, Optional[str]], args: Iterable[str]) -> bool:
        return all([data.get(x) is None for x in args])

    @classmethod
    def is_unambiguous(cls, data: Dict[str, Optional[str]], schema: str) -> bool:
        ...
        if schema == "prebaked":
            ...
            return cls.are_args_none(data, ["recipe", "recipe_args", "recipe_kwargs"])
        elif schema == "recipe":  # prebaked, command
            return cls.are_args_none(data, ["command"])
        return False

    @classmethod
    def ensure_unambiguity(
        cls, data: Dict[str, Optional[str]], is_prebaked_data: bool
    ...
    @classmethod
    def load_prebaked_projects(cls, prebaked_dir: Optional[str]) -> Dict[Any, Any]:
        ...
        logger.info(f"{len(found)} prebaked projects found{suffix}")
        return o
    ...
    @classmethod
    def is_recipe(cls, filename: str, exclude_pattern: Optional[str] = None) -> bool:

        if not filename.endswith(".py"):
            return False

        if exclude_pattern is None or not re.search(
            pattern=exclude_pattern, string=filename
        ):
            return True
    ...
    @classmethod
    def import_recipes(
        cls, recipe_dir: Optional[str], exclude_pattern: Optional[str] = "^(_|loader).*"
    ) -> set:
        ...
        if recipe_dir:
            logger.info(f"Looking for recipes in {os.path.abspath(recipe_dir)}")
        ...
        suffix = f": {sorted(found)}" if found else ""
        logger.info(f"{len(found)} additional recipes found{suffix}")
...
class ProcessRegistry(object):
    ...
    def setup_scheduled_cleaning(self, interval: Optional[int], timeout: Optional[int]):
    ...
    def catch_signal(self, signum: int, frame: typing.types.FrameType):
    ...
    def scheduled_cleaning(self, interval: int, timeout: int):
        ...
        if not self.scheduled_stopper.is_set():
            self.current_timer = threading.Timer(
                interval, self.scheduled_cleaning, (interval, timeout)
            )
    ...
    def register_process(
        ...
        expiry_timestamp: int = 8_000_000_000,
    ) -> None:
        ...
        cur = self.connection.cursor()
        cur.execute(stmt, (identifier, session_name, port, pid, round(expiry_timestamp)))
        self.connection.commit()
    ...
    def find_process_from_registry(
        self, allow_unsafe: bool = False, **kwargs
    ...
    def delete_process_from_registry(self, allow_unsafe: bool = False, **kwargs) -> None:
    ...
    def already_has_instance(self, identifier: str) -> bool:
        if self.find_process_from_registry(identifier=identifier):
            logger.error(f"{identifier} already started a Prodigy instance")
            return True
        return False
...
class Progeny(object):
    ...
    def __init__(
        ...
        self.registry = ProcessRegistry(
            self.port_manager, scheduled_cleaning_interval, scheduled_cleaning_timeout
        )
    ...
    def get_process(
        self, command: str, config: Dict[str, Any], session_name: str
    ...
    def get_command_and_config(
        ...
        if command:
            ...
            command = self.adapter.parse_command(
                recipe=recipe, recipe_args=recipe_args, recipe_kwargs=recipe_kwargs
            )
    ...
    def spin(
        ...
        self.registry.register_process(identifier, session_name, port, process)
